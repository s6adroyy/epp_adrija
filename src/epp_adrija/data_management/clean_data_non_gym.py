...
def process_data(input_dataframe):
    ...
    input_dataframe.loc[~input_dataframe["psample"].isin(migration_info), :].copy()
    input_dataframe["Age"] = input_dataframe["syear"] - input_dataframe["jl0233"]
    input_dataframe.loc[(input_dataframe["Age"] == 17), :].copy()
    input_dataframe.loc[(input_dataframe["jl0164"].isin(["[2] Nein"])), :].copy()
...
def filter_based_on_school(df1):
    ...
    df1.rename(columns={"bula_h": "State"})
    ...
    df1.loc[~df1["State"].isin(states_list2), :]
...
def rename_variables(df1):
    ...
    for var in trust_variables:
        df1 = df1.loc[~df1[var].isin(["[-1] keine Angabe"])]
    df1.rename(
        columns={
            "jl0361": "trust",
            "jl0362": "rely_none",
...
def rename_independent_var(df2):
    ...
    df2 = df2.loc[~df2["jl0272"].isin(["[-1] keine Angabe"])]
    ...
    df2 = df2.loc[~df2["jl0151"].isin(["[-1] keine Angabe"])]
    ...
    df2 = df2.loc[~df2["freli"].isin(["[-1] keine Angabe"])]
    ...
    df2 = df2.loc[~df2["mreli"].isin(["[-1] keine Angabe"])]
